...
@Component({
  selector: 'app-state-wise-count',
  templateUrl: './state-wise-count.component.html',
  styleUrls: ['./state-wise-count.component.scss']
})
export class StateWiseCountComponent implements OnInit {
  ...
  getDistributorList(action:any) 
    ...
    getStateWiseReport(action:any) 
    {
        ...
        this.db.post_rqst(  {'filter': this.filter}, 'master/state_wise_report')
        .subscribe( d => {
            this.loading_list = false;
            console.log(d);            
            this.state_wise_count = d.karigar_report;            
             
        });
      this.State_wise_total();
    ...
    {
        this.filter.mode = 1;
        this.db.post_rqst(  {'filter': this.filter , 'login':this.db.datauser}, 'master/export_state_wise_report')
        .subscribe( d => {
    ...
    exportDistributor()
    {
        ...
        .subscribe( d => {
            document.location.href = this.db.myurl+'/app/uploads/exports/Customers.csv';
            console.log(d);
        });
    }
    ...
    State_wise_total()
    {
        this.db.post_rqst('','master/state_wise_report_total')
        .subscribe(d => {
            console.log(d);
            this.karigar_report = d.karigar_report;
        });
    }
