...
import { DistributoruploadComponent } from 'src/app/upload/distributorupload/distributorupload.component';
  ...
  getDistributorList(action:any) 
    {
        ...
        this.db.post_rqst(  {'filter': this.filter , 'login':this.db.datauser,user_type:"3"}, 'master/distributor_list?page=' + this.current_page)
        .subscribe( d => {
            this.loading_list = false;
            console.log(d);            
            this.current_page = d.distributor_list.current_page;
            this.last_page = d.distributor_list.last_page;
            this.total_distributors =d.distributor_list.total;
            this.distributors = d.distributor_list.data;            
    ...
    {
        this.filter.mode = 1;
        this.db.post_rqst(  {'filter': this.filter , 'login':this.db.datauser,user_type:'3'}, 'master/export_distributor')
        .subscribe( d => {
            document.location.href = this.db.myurl+'/app/uploads/distributors.csv';
            console.log(d);
        });
    }
    ...
    deleteDistributor(id)
    {
        ...
        .then((result) => {
            if(result)
            {
                this.db.post_rqst({'distributor_id': id}, 'master/delete_distributor')
                .subscribe(d => {
                    console.log(d);
                    this.getDistributorList('');
    ...
    edit(id){
        this.router.navigate(['/distributor-add/' + id]);
    }
    ...
    downloadSamplefile = () => {
        ...
        .subscribe( d => {
            document.location.href = this.db.myurl+'/app/uploads/exports/distributor_upload_sample.csv';
            //console.log(d);
        });
    }

    bulkUpload = () => {
        const dialogRef = this.alrt.open(DistributoruploadComponent,{
            width: '500px',
