...
import { DealeruploadComponent } from '../dealerupload/dealerupload.component';
...
export class DealerListComponent implements OnInit {
  ...
  getDealerList(action:any) 
    ...
    exportDealer()
    {
        ...
        .subscribe( d => {
            document.location.href = this.db.myurl+'/app/uploads/exports/Dealers.csv';
            console.log(d);
        });
    }
    ...
    downloadDealer = (id) => {
        this.filter.mode = 1;
        this.db.post_rqst(  {'id': id}, 'karigar/exportDealerKarigar')
        .subscribe( d => {
            document.location.href = this.db.myurl+'/app/uploads/exports/Dealers.csv';
            console.log(d);
        });
    ...
    modalOpen(karigar_id){
        console.log("dealer id =====>",karigar_id)
        const dialogRef = this.alrt.open(AssignSlaesComponent,
            {
                width: '500px',
                height: '200px',
    ...
    bulkUpload = () => {
        const dialogRef = this.alrt.open(DealeruploadComponent,{
          width: '500px',
