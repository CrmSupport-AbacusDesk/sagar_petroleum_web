...
export class CouponCodeListComponent implements OnInit {
    ...
    getScannedList(action) 
    {
        ...
        .subscribe( d => {
            ...
            this.perPage = d.scanned_coupon.per_page;   
            this.sr_no = this.current_page - 1;
            this.sr_no = this.sr_no * this.perPage; 
    ...
    exportRetailerCouponList()
    {
        ...
        this.filter.mode = 1;
        this.db.post_rqst(  {'filter': this.filter , 'login':this.db.datauser}, 'offer/exportScannedCouponListRetailer')
        .subscribe( d => {
            this.loading_list = false;
            document.location.href = this.db.myurl+'app/uploads/exports/scanned-coupon-list-dealer.csv';
            console.log(d);
        });
    }
